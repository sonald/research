...
from .models import ActorCritic, QNetwork, mask_logits, masked_argmax, observation_to_tensors
...
def load_policy(path: str | Path, device: torch.device) -> tuple[str, ExperimentConfig, torch.nn.Module]:
    ...
    if algo == "dqn":
        ...
        model = QNetwork(hidden_dim=config.dqn.hidden_dim).to(device)
    elif algo == "a2c":
        assert config.a2c is not None
        model = ActorCritic(hidden_dim=config.a2c.hidden_dim).to(device)
    else:
...
def policy_action(
    ...
    *,
    ...
    board, meta, action_mask = observation_to_tensors(observation, device)
    with torch.no_grad():
        if algo == "dqn":
            q_values = model(board, meta)
            return int(masked_argmax(q_values, action_mask).item())

        output = model(board, meta)
        masked_logits = mask_logits(output.logits, action_mask)
        if stochastic:
