"""Reference A2C trainer for the Tetris baseline."""
...
from .models import ActorCritic, mask_logits, observation_to_tensors
...
class A2CTrainer:
    ...
    def train(self) -> dict[str, Any]:
        ...
        while self.global_step < self.train_config.total_steps:
            rollout_board: list[np.ndarray] = []
            rollout_meta: list[np.ndarray] = []
            ...
            for _ in range(self.algo_config.n_steps):
                board, meta, action_mask = observation_to_tensors(observation, self.device)
                with torch.no_grad():
                    output = self.model(board, meta)
                    masked_policy_logits = mask_logits(output.logits, action_mask)
                    distribution = Categorical(logits=masked_policy_logits)
                    actions = distribution.sample()

                next_observation, rewards, terminated, truncated, _ = self.envs.step(actions.cpu().numpy())
                dones = np.logical_or(terminated, truncated)

                rollout_board.append(np.asarray(observation["board"], dtype=np.float32))
                rollout_meta.append(np.asarray(observation["meta"], dtype=np.float32))
                rollout_masks.append(np.asarray(observation["action_mask"], dtype=bool))
            ...
            board, meta, action_mask = observation_to_tensors(observation, self.device)
            with torch.no_grad():
                bootstrap = self.model(board, meta).value
            ...
            flat_board = torch.as_tensor(np.concatenate(rollout_board, axis=0), dtype=torch.float32, device=self.device)
            flat_meta = torch.as_tensor(np.concatenate(rollout_meta, axis=0), dtype=torch.float32, device=self.device)
            flat_masks = torch.as_tensor(np.concatenate(rollout_masks, axis=0), dtype=torch.bool, device=self.device)
            flat_actions = torch.as_tensor(np.concatenate(rollout_actions, axis=0), dtype=torch.long, device=self.device)
            flat_returns = returns.reshape(-1)

            output = self.model(flat_board, flat_meta)
            distribution = Categorical(logits=mask_logits(output.logits, flat_masks))
